...
class PayzaService
{
    ...
    {
        $result = sprintf("USER=%s&PASSWORD=%s&AMOUNT=%s&CURRENCY=%s&RECEIVEREMAIL=%s&SENDEREMAIL=%s&PURCHASETYPE=%s&NOTE=%s&TESTMODE=%s",
            urlencode($this->username), urlencode($this->password), urlencode((string)$amountPaid), urlencode($currency), urlencode($receiverEmail), urlencode($senderEmail), 
    ...
    {
        $result = sprintf("USER=%s&PASSWORD=%s&CURRENCY=%s",
            urlencode($this->username), urlencode($this->password), urlencode($currency));
        return $result;
    }
